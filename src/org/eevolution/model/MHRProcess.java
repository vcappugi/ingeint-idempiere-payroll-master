...
public class MHRProcess extends X_HR_Process implements DocAction {
	...
	private void createMovements() throws Exception {
		...
		//add extended where
		String sqldel ="DELETE FROM HR_Movement m WHERE HR_Process_ID=? AND IsRegistered<>? AND AD_Org_ID=?";
		if (getC_BPartner_ID() >0)
			sqldel= sqldel + " AND C_BPartner_ID=" + getC_BPartner_ID();
		if (getHR_Job_ID()>0)
			sqldel= sqldel + " AND HR_Job_ID=" + getHR_Job_ID();
		if (getHR_Department_ID()>0)
			sqldel= sqldel + " AND HR_Department_ID=" + getHR_Department_ID();
		
		
			
		int no = DB
				.executeUpdateEx(sqldel,
						new Object[] { getHR_Process_ID(), true, getAD_Org_ID() },
						get_TrxName());
